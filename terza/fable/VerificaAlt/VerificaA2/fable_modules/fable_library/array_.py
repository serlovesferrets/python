...
def try_last(array: Array[_T]) -> _T | None:
    if len(array) == 0:
        return None
...
def map_indexed2(
    f: Callable[[int, _T1, _T2], _U], source1: Array[_T1], source2: Array[_T2], cons: Cons_1[_U]
) -> Array[_U]:
...
def map3(
    f: Callable[[_T1, _T2, _T3], _U], source1: Array[_T1], source2: Array[_T2], source3: Array[_T3], cons: Cons_1[_U]
) -> Array[_U]:
    if True if (len(source1) != len(source2)) else (len(source2) != len(source3)):
        raise Exception("Arrays had different lengths")
...
def map_fold(
    mapping: Callable[[_STATE, _T], tuple[_RESULT, _STATE]], state: _STATE, array: Array[_T], cons: Cons_1[_RESULT]
...
def map_fold_back(
    mapping: Callable[[_T, _STATE], tuple[_RESULT, _STATE]], array: Array[_T], state: _STATE, cons: Cons_1[_RESULT]
) -> tuple[Array[_RESULT], _STATE]:
    match_value: int = len(array) or 0
    if match_value == 0:
        return ([], state)
...
def truncate(count: int, array: Array[_T]) -> Array[_T]:
    def _arrow43(x: int, y: int, count: Any = count, array: Any = array) -> int:
...
def index_of(array: Array[_T], item_1: _T, start: int | None, count: int | None, eq: IEqualityComparer_1[Any]) -> int:
...
def scan(
    folder: Callable[[_STATE, _T], _STATE], state: _STATE, array: Array[_T], cons: Cons_1[_STATE]
) -> Array[_STATE]:
...
def scan_back(
    folder: Callable[[_T, _STATE], _STATE], array: Array[_T], state: _STATE, cons: Cons_1[_STATE]
) -> Array[_STATE]:
    res: Array[_STATE] = Helpers_allocateArrayFromCons(cons, len(array) + 1)
    res[len(array)] = state
    for i in range(len(array) - 1, 0 - 1, -1):
...
def insert_range_in_place(index: int, range: IEnumerable_1[_T], array: Array[_T]) -> None:
    ...
    with get_enumerator(range) as enumerator:
        while enumerator.System_Collections_IEnumerator_MoveNext():
            ...
            def _arrow44(__unit: None = None, index: Any = index, range: Any = range, array: Any = array) -> Array[_T]:
                array_1: Array[_T] = array
                index_1: int = i or 0
...
def try_find(predicate: Callable[[_T], bool], array: Array[_T]) -> _T | None:
...
def try_find_index(predicate: Callable[[_T], bool], array: Array[_T]) -> int | None:
    match_value: int = (next((i for i, x in enumerate(array) if (predicate)(x)), -1)) or 0
    if match_value > -1:
        return match_value
...
def pick(chooser: Callable[[__A], __B | None], array: Array[__A]) -> __B:
...
def try_pick(chooser: Callable[[__A], __B | None], array: Array[__A]) -> __B | None:
...
def try_find_back(predicate: Callable[[__A], bool], array: Array[__A]) -> __A | None:
...
def try_find_index_back(predicate: Callable[[__A], bool], array: Array[__A]) -> int | None:
...
def choose(chooser: Callable[[_T], _U | None], array: Array[_T], cons: Cons_1[_U]) -> Array[_U]:
    res: Array[_U] = []
    for i in range(0, (len(array) - 1) + 1, 1):
    ...
    if equals_1(cons, None):
        ...
        def _arrow45(x: _U | None = None, chooser: Any = chooser, array: Any = array, cons: Any = cons) -> _U | None:
            return x
...
def fold(folder: Callable[[_STATE, _T], _STATE], state: _STATE, array: Array[_T]) -> _STATE:
    def _arrow47(acc: _STATE, x: _T, folder: Any = folder, state: Any = state, array: Any = array) -> _STATE:
        return folder(acc, x)
...
def iterate2(action: Callable[[_T1, _T2], None], array1: Array[_T1], array2: Array[_T2]) -> None:
...
def iterate_indexed2(action: Callable[[int, _T1, _T2], None], array1: Array[_T1], array2: Array[_T2]) -> None:
    if len(array1) != len(array2):
        different_lengths()
...
def permute(f: Callable[[int], int], array: Array[_T]) -> Array[_T]:
    ...
    def _arrow48(i: int, x: _T, f: Any = f, array: Any = array) -> None:
        j: int = f(i) or 0
...
def set_slice(target: Array[_T], lower: int | None, upper: int | None, source: Array[_T]) -> None:
...
def sort(xs: Array[_T], comparer: IComparer_1[_T]) -> Array[_T]:
    ...
    def comparer_1(x: _T, y: _T, xs: Any = xs, comparer: Any = comparer) -> int:
        return comparer.Compare(x, y)
...
def sort_by(projection: Callable[[_A], _B], xs: Array[_A], comparer: IComparer_1[_B]) -> Array[_A]:
    ...
    def comparer_1(x: _A, y: _A, projection: Any = projection, xs: Any = xs, comparer: Any = comparer) -> int:
        return comparer.Compare(projection(x), projection(y))
...
def unfold(generator: Callable[[_STATE], tuple[_T, _STATE] | None], state: _STATE) -> Array[_T]:
    res: Array[_T] = []
...
def zip3(array1: Array[_T], array2: Array[_U], array3: Array[_V]) -> Array[tuple[_T, _U, _V]]:
...
def try_exactly_one(array: Array[_T]) -> _T | None:
    if len(array) == 1:
        return some(array[0])
...
def try_head(array: Array[_T]) -> _T | None:
    if len(array) == 0:
        return None
...
def try_item(index: int, array: Array[_T]) -> _T | None:
    if True if (index < 0) else (index >= len(array)):
        return None
...
def fold_back(folder: Callable[[_T, _STATE], _STATE], array: Array[_T], state: _STATE) -> _STATE:
    def _arrow54(acc: _STATE, x: _T, folder: Any = folder, array: Any = array, state: Any = state) -> _STATE:
        return folder(x, acc)
...
def fold_indexed2(
    folder: Callable[[int, __A, __B, __C], __A], state: __A, array1: Array[__B], array2: Array[__C]
) -> __A:
    acc: __A = state
    if len(array1) != len(array2):
...
def fold2(
    folder: Callable[[_STATE, _T1, _T2], _STATE], state: _STATE, array1: Array[_T1], array2: Array[_T2]
) -> _STATE:
    def _arrow55(
        _arg: int,
        acc: _STATE,
...
def fold_back_indexed2(
    folder: Callable[[int, _T1, _T2, _STATE], _STATE], array1: Array[_T1], array2: Array[_T2], state: _STATE
) -> _STATE:
    acc: _STATE = state
    if len(array1) != len(array2):
        different_lengths()
...
def fold_back2(
    f: Callable[[_T1, _T2, _STATE], _STATE], array1: Array[_T1], array2: Array[_T2], state: _STATE
) -> _STATE:
    def _arrow56(
        _arg: int,
        x: _T1,
...
def exists_offset2(
    predicate_mut: Callable[[__A, __B], bool], array1_mut: Array[__A], array2_mut: Array[__B], index_mut: int
) -> bool:
    while True:
...
def max_by(projection: Callable[[_A], _B], xs: Array[_A], comparer: IComparer_1[_B]) -> _A:
    def _arrow58(x: _A, y: _A, projection: Any = projection, xs: Any = xs, comparer: Any = comparer) -> _A:
...
def max(xs: Array[_A], comparer: IComparer_1[_A]) -> _A:
    def _arrow59(x: _A, y: _A, xs: Any = xs, comparer: Any = comparer) -> _A:
...
def min_by(projection: Callable[[_A], _B], xs: Array[_A], comparer: IComparer_1[_B]) -> _A:
    def _arrow60(x: _A, y: _A, projection: Any = projection, xs: Any = xs, comparer: Any = comparer) -> _A:
...
def min(xs: Array[_A], comparer: IComparer_1[_A]) -> _A:
    def _arrow61(x: _A, y: _A, xs: Any = xs, comparer: Any = comparer) -> _A:
...
def windowed(window_size: int, source: Array[_T]) -> Array[Array[_T]]:
    ...
    def _arrow62(x: int, y: int, window_size: Any = window_size, source: Any = source) -> int:
        return compare_primitives(x, y)
...
def split_into(chunks: int, array: Array[_T]) -> Array[Array[_T]]:
    ...
    if len(array) == 0:
        ...
        def _arrow63(x: int, y: int, chunks: Any = chunks, array: Any = array) -> int:
            return compare_primitives(x, y)
        ...
        for i in range(0, (chunks_1 - 1) + 1, 1):
            ...
            def _arrow64(x_1: int, y_1: int, chunks: Any = chunks, array: Any = array) -> int:
